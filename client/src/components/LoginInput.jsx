...
const LoginInput = ({placeHolder, icon, inputState, inputStateFunc, type, isSignUp}) => {
  ...
  return (
    <motion.div  {...fadeInOut} className={`flex items-center justify-center gap-3 bg-lightOverlay backdrop-blur-md rounded-md w-full  px-3 py-2 ${isFocus ? "shadow-md shadow-red-400" : "shadow-none"}`}>
      {icon}
      <input type={type}
