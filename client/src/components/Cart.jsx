...
import { baseURL, getAllCartItems, increamentItemQuantity } from "../api";
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6 gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            ...
            {/* total */}
            <div className="bg-zinc-800 rounded-t-[60px] w-full h-[35%] flex flex-col items-center justify-center px-4 py-6 gap-4">
              ...
              <motion.div
                {...buttonClick}
                className="mb-[100px] bg-orange-400 w-[70%] px-4 py-3 text-xl flex justify-center text-headingColor font-semibold hover:bg-orange-500 drop-shadow-md rounded-2xl"
          ...
          <>
            ...
            <div className="w-full flex flex-col items-center justify-center">
              <img src={NotFound} className="h-340 mt-[100px]" />
...
export const CartItemCard = ({ index, data }) => {
  ...
  const decrementCart = (productId) => {
    ...
    increamentItemQuantity(user?.user_id, productId, "decrement").then(
      (data) => {
  ...
  const incrementCart = (productId) => {
    dispatch(alertSuccess("Updated the cartitem"));
    increamentItemQuantity(user?.user_id, productId, "increment").then(
      (data) => {
        getAllCartItems(user?.user_id).then((items) => {
          dispatch(setCartItems(items));
          dispatch(alertNull());
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-start gap-1 w-full">
        ...
        <p className="text-sm flex items-center justify-center gap-1 font-semibold text-red-400 md:ml-auto sm:-mr-1 sm:ml-3">
          <HiCurrencyRupee className="text-red-400 " /> {itemTotal}
        </p>
      </div>

      <div className="md:ml-auto flex items-center justify-center gap-3 sm:mt-12 sm:-ml-32">
        <motion.div
          {...buttonClick}
          onClick={() => decrementCart(data?.productId)}
          className="md:w-8 md:h-8 w-4 h-4 flex items-center justify-center rounded-md drop-shadow-md bg-zinc-900 cursor-pointer"
        >
          <p className="md:text-xl text-md font-semibold text-primary">--</p>
        </motion.div>
        <p className="md:text-lg text-md text-primary font-semibold">
          {data?.quantity}
        </p>
        <motion.div
          {...buttonClick}
